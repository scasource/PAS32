...
type
  TSalesPageForm = class(TForm)
    ...
    btnConditionNotes: TButton;
    ConditionNotesDialog: TwwMemoDialog;
    ...
    procedure btnConditionNotesClick(Sender: TObject);
...
uses GlblVars, WinUtils, PASUTILS, UTILEXSD,  Utilitys, OwnerProgressionDialog,
     OwnerProgressionDialog_New, DataAccessUnit,
     GlblCnst;
  ...
  If (Deblank(SwisSBLKey) <> '')
    then
      begin
        ...
        If ((not glblAlwaysShowMostCurrentSaleOnSalesPage) and
            (SalesNumber <> 0) and
  ...
  with MainTable do
    begin
      ...
      FieldByName('OldOwnerName').Text := OrigOldOwnerName;
        ...
        {FXX03102010-1(2.22.2.4)[I7099]: East and north coordinates were switched.}
...
(*      FieldByName('EastCoord').AsInteger := TempParcelTable.FieldByName('GridCordNorth').AsInteger;
      ...
      FieldByName('NorthCoord').AsInteger := TempParcelTable.FieldByName('GridCordNorth').AsInteger;
      FieldByName('EastCoord').AsInteger := TempParcelTable.FieldByName('GridCordEast').AsInteger;
  ...
  If ((not FormIsInitializing) and
    ...
    then
      begin
        ...
        OriginalSalesDate := MainTable.FieldByName('SaleDate').AsDateTime;
...
begin
  ValidSale := True;
    ...
    {FXX04262007-1(2.11.1.26): Only fill in the sales information on an edit if the sale date changed.}

  If ((MainTable.State = dsInsert) or
      ((MainTable.State = dsEdit) and
       _Compare(OriginalSalesDate, MainTable.FieldByName('SaleDate').AsDateTime, coNotEqual)))
    then
      begin
...
begin
  If (GlblAskSave and
      ...
      _Compare(MainTable.FieldByName('SaleDate').AsString, coNotBlank))
    then MainTable.Post;
...
var
  ...
  ExString, sSaleAssessmentYear : String;
  TYResSiteFound, TYComSiteFound,
  NYResSiteFound, NYComSiteFound,
  ResSiteFound, ComSiteFound : Boolean;
    ...
    then
      begin
        ...
        If (TYResSiteFound or TYComSiteFound)
          ...
          {FXX06102010-1(2.26.1)[I7462]: If a sale applies to an old year, it should look in history for the inventory.}

        sSaleAssessmentYear := MainTable.FieldByName('SaleAssessmentYear').AsString;
        If _Compare(sSaleAssessmentYear, GlblThisYear, coLessThan)
        then
        begin
          TempTaxRollYr := '';

          ResSiteFound := SiteFound(ResidentialSiteTableName, SwisSBLKey,
                                    History, sSaleAssessmentYear);
          ComSiteFound := SiteFound(CommercialSiteTableName, SwisSBLKey,
                                    History, sSaleAssessmentYear);

          If (ResSiteFound or ComSiteFound)
          then
          begin
            ProcessingType := History;
            TempTaxRollYr := sSaleAssessmentYear;
          end;

        end;  {If _Compare(MainTable.FieldByName('TaxSaleYear').AsString, GlblThisYear, coLessThan)}
        ...
        If NameAddressInfoChanged(OrigNameAddressRec,
                                  NewNameAddressRec)
          then
          begin
            ...
            then AutoUpdateNamesAndAddressesInOtherSystems(NYParcelTable,
                                                           SwisSBLKey,
                                                           GlblNextYear);

          end;  {If NameAddressInfoChanged}
        ...
        If _Compare(NYParcelTable.FieldByName('BankCode').AsString, coNotBlank)
        then MessageDlg('This parcel has bank code ' +  NYParcelTable.FieldByName('BankCode').AsString + '.' + #13 +
...
begin
  If glblUseNewOwnerProgressionForm
  then
  begin
    try
      SalesOwnerProgressionForm_New := TSalesOwnerProgressionForm_New.Create(nil);

      with MainTable do
        SalesOwnerProgressionForm_New.InitializeForm(SwisSBLKey,
                                                 FieldByName('DeedBook').AsString,
                                                 FieldByName('DeedPage').AsString,
                                                 FieldByName('DeedDate').AsString);
      SalesOwnerProgressionForm_New.ShowModal;
    finally
      SalesOwnerProgressionForm_New.Free;
    end;
...
{======================================================================}
Procedure TSalesPageForm.btnConditionNotesClick(Sender: TObject);
...
begin
  ...
  ConditionNotesDialog.Caption := 'Condition Notes for Sale #' + MainTable.FieldByName('SaleNumber').Text;
  ConditionNotesDialog.Execute;
end;
