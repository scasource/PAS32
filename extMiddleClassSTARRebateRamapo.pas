unit extMiddleClassSTARRebateRamapo;
...
type
  ...
  public
    ...
    Function ParcelMeetsChangeCriteria(    tbParcels : TTable;
                                       ...
                                       var sOriginalSTARCode : String;
                                       var iOriginalChangeType : Integer) : Boolean;
    ...
    Procedure InsertOneSortRecord(tbSort : TTable;
                                  ...
                                  iChangeType : Integer;
                                  sOriginalSTARCode : String;
                                  iOriginalChangeType : Integer;
...
{==============================================================}
Procedure CheckForSTARChanges_Original(    tbExemptions : TTable;
                                  tbBillExemptions : TTable;
                                  sSwisSBLKey : String;
                                  sSchoolCode : String;
                                  sAssessmentYear : String;
  ...
  If LocateParcelInBillingFile(tbBillParcels, sSwisSBLKey, sSchoolCode,
    ...
    then
      begin
        ...
        CheckForSTARChanges_Original(tbExemptions, tbBillExemptions, sSwisSBLKey, sSchoolCode,
                            sAssessmentYear, sBillingYear, sCollectionType,
                            slSchoolCollections, sOriginalSTARCode, iOriginalChangeType);
        ...
        If (_Compare(sOriginalSTARCode, coNotBlank) and
            _Compare(iOriginalChangeType, ctNone, coEqual))
          then CheckForOwnerOrAddressChange(tbTempParcels, tbBillParcels, iOriginalChangeType);
...
var
  sSTARCode, sChangeType, sOriginalChangeType : String;
  tbTempParcels : TTable;
...
begin
  ...
  case iOriginalChangeType of
    ctSTARChanged_BasicToEnhanced,
    ctSTARChanged_EnhancedToBasic : sOriginalChangeType := 'X';
    ctSTARAdded, ctNewParcel : sOriginalChangeType := 'A';
    ctSTARRemoved, ctParcelInactivated : sOriginalChangeType := 'D';
    else sOriginalChangeType := 'C';
  end;
  ...
  with tbSort do
    try
      ...
      FieldByName('OrigSTARCode').AsString := sOriginalSTARCode;
      FieldByName('OrigChangeCode').AsString := sOriginalChangeType;
      FieldByName('OrigChangeType').AsInteger := iOriginalChangeType;
      Post;
    except
    end;
...
var
  sSwisSBLKey, sCurrentSTARCode, sOriginalSTARCode, sPriorSTARCode : String;
  iChangeType, iOriginalChangeType : Integer;
      ...
      begin
        ...
        If (ParcelMeetsBaseCriteria(tbParcels, sAssessmentYear,
            ...
            ParcelMeetsChangeCriteria(tbParcels, tbExemptions,
                                      ...
                                      sCurrentSTARCode, iChangeType,
                                      sOriginalSTARCode, iOriginalChangeType))
                  ...
                  begin
                    ...
                    If _Compare(sPriorSTARCode, coNotBlank)
                      then InsertOneSortRecord(tbSort, tbBillParcels, tbNYParcels,
                                               sSwisSBLKey, sAssessmentYear,
                                               sPriorSTARCode, ctParcelInactivated,
                                               sOriginalSTARCode, iOriginalChangeType,
                                               psBillingParcels);
      ...
      If bPrintDetails
        then
          begin
            ...
            SetTab(1.5, pjCenter, 2.2, 5, BoxLineAll, 25);  {Owner}
            SetTab(3.7, pjCenter, 0.8, 5, BoxLineAll, 25);  {STAR Type}
            SetTab(4.5, pjCenter, 1.0, 5, BoxLineAll, 25);  {Change Type}
            SetTab(5.5, pjCenter, 0.5, 5, BoxLineAll, 25);  {STAR Type}
            SetTab(6.0, pjCenter, 1.6, 5, BoxLineAll, 25);  {Change Type}
            SetTab(7.6, pjCenter, 0.5, 5, BoxLineAll, 25);  {Difference?}
            ...
            SetTab(1.5, pjLeft, 2.2, 5, BoxLineAll, 0);  {Owner}
            SetTab(3.7, pjLeft, 0.8, 5, BoxLineAll, 0);  {STAR Type}
            SetTab(4.5, pjLeft, 1.0, 5, BoxLineAll, 0);  {Change Type}
            SetTab(5.5, pjLeft, 0.5, 5, BoxLineAll, 0);  {STAR Type}
            SetTab(6.0, pjLeft, 1.6, 5, BoxLineAll, 0);  {Change Type}
            SetTab(7.6, pjCenter, 0.5, 5, BoxLineAll, 0);  {Difference?}
...
begin
  ...
  with tbSort do
    while not (EOF or bReportCancelled) do
      begin
        ...
        with Sender as TBaseReport do
          begin
            bDifference := _Compare(FieldByName('ChangeType').AsInteger,
                                    FieldByName('OrigChangeType').AsInteger,
                                    coNotEqual);

            If bDifference
              then inc(iDifferenceCount);

            If bPrintDetails
              then
                begin
                  Print(#9 + FieldByName('Print_Key').AsString +
                        #9 + FieldByName('Last_Name_1').AsString +
                        #9 + FieldByName('OrigSTARCode').AsString +
                        #9 + GetChangeTypeDescription(FieldByName('OrigChangeType').AsInteger) +
                        #9 + GetSTARCode(FieldByName('STAR_EX_Type').AsInteger) +
                        #9 + GetChangeTypeDescription(FieldByName('ChangeType').AsInteger));

                  If (bDifference and
                      _Compare(FieldByName('ChangeType').AsInteger, ctSTARRemoved, coEqual))
                    then Println(#9 + '***')
                    else Println(#9 + BooleanToX_Blank(bDifference));

                end;  {If bPrintDetails}
  ...
  If not bReportCancelled
    then
      begin
        PrintTotals(Sender, rTotalsRec);

        with Sender as TBaseReport do
          begin
            Println('');
            Println('Differences: ' + IntToStr(iDifferenceCount));
          end;  {with Sender as TBaseReport do}
