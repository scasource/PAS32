object CertiorariCondoCopyDialog: TCertiorariCondoCopyDialog
  Left = 446
  Top = 123
  Width = 332
  Height = 462
  Caption = 'Please enter destination parcels'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Arial'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 16
  object InformationLabel: TLabel
    Left = 10
    Top = 11
    Width = 305
    Height = 71
    AutoSize = False
    Caption = 
      'Please enter the additional parcels that should have a certiorar' +
      'i created ...'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    WordWrap = True
  end
  object OKButton: TBitBtn
    Left = 68
    Top = 392
    Width = 86
    Height = 33
    Caption = 'OK'
    TabOrder = 0
    OnClick = OKButtonClick
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object CancelButton: TBitBtn
    Left = 172
    Top = 393
    Width = 86
    Height = 33
    TabOrder = 1
    Kind = bkCancel
  end
  object ParcelGroupBox: TGroupBox
    Left = 10
    Top = 75
    Width = 305
    Height = 308
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    object ParcelGrid: TStringGrid
      Left = 44
      Top = 20
      Width = 218
      Height = 235
      Hint = 
        'Please enter a parcel ID in the form ss/SBL where ss = short swi' +
        's code.'
      ColCount = 1
      DefaultColWidth = 200
      DefaultRowHeight = 20
      FixedCols = 0
      RowCount = 100
      FixedRows = 0
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goEditing, goTabs, goAlwaysShowEditor]
      ParentFont = False
      ScrollBars = ssVertical
      TabOrder = 0
    end
    object ClearButton: TBitBtn
      Left = 29
      Top = 265
      Width = 73
      Height = 33
      Caption = '&Clear'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = ClearButtonClick
      Glyph.Data = {
        
        05555777775555557F5555000555555505555577755555557555}
      NumGlyphs = 2
      IsControl = True
    end
    object AddButton: TBitBtn
      Left = 116
      Top = 265
      Width = 73
      Height = 33
      Caption = '&Add'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = AddButtonClick
      Glyph.Data = {
        
        00333377737FFFFF773333303300000003333337337777777333}
      NumGlyphs = 2
      IsControl = True
    end
    object DeleteButton: TBitBtn
      Left = 203
      Top = 265
      Width = 73
      Height = 33
      Caption = '&Delete'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      OnClick = DeleteButtonClick
      Glyph.Data = {
        
        3333333337FFF7F3333333333000003333333333377777333333}
      NumGlyphs = 2
      IsControl = True
    end
  end
  object CertiorariTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY_CERTNUM'
    TableName = 'GCertiorariTable'
    TableType = ttDBase
    Left = 178
    Top = 191
  end
  object CurrentExemptionsTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY'
    TableType = ttDBase
    Left = 190
    Top = 284
  end
  object CertiorariExemptionsTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYYEAR_SWISSBL_CERTNUM_EXCODE'
    TableName = 'gcpexemptionrec'
    TableType = ttDBase
    Left = 53
    Top = 199
  end
  object CurrentAssessmentTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY'
    TableType = ttDBase
    Left = 145
    Top = 55
  end
  object CertiorariExemptionsAskedTable: TwwTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYYEAR_SBL_CERTNUM_EX'
    TableName = 'gcexemptionsasked'
    TableType = ttDBase
    ControlType.Strings = (
      'ExemptionCode;CustomEdit;ExemptionCodeAskedLookupCombo')
    SyncSQLByRange = False
    NarrowSearch = False
    ValidateWithMask = True
    Left = 76
    Top = 51
  end
  object CertiorariSpecialDistrictsTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYYEAR_SWISSBL_CERTNUM_SD'
    TableName = 'gcpspcldistrec'
    TableType = ttDBase
    Left = 126
    Top = 345
  end
  object CurrentSpecialDistrictsTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY'
    TableType = ttDBase
    Left = 60
    Top = 115
  end
  object ParcelTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY'
    TableName = 'TParcelRec'
    TableType = ttDBase
    Left = 230
    Top = 51
  end
  object SwisCodeTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYSWISCODE'
    TableType = ttDBase
    Left = 77
    Top = 274
  end
  object PriorAssessmentTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY'
    TableType = ttDBase
    Left = 165
    Top = 129
  end
  object LawyerCodeTable: TTable
    DatabaseName = 'PASsystem'
    IndexFieldNames = 'Code'
    TableName = 'ZGLawyerCodes'
    TableType = ttDBase
    Left = 137
    Top = 386
  end
  object CertiorariLookupTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY_CERTNUM'
    TableName = 'GCertiorariTable'
    TableType = ttDBase
    Left = 46
    Top = 327
  end
  object CertiorariExemptionsAskedLookupTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYYEAR_SBL_CERTNUM_EX'
    TableName = 'gcexemptionsasked'
    TableType = ttDBase
    Left = 159
    Top = 243
  end
  object GrievanceTable: TTable
    DatabaseName = 'PASsystem'
    IndexName = 'BYTAXROLLYR_SWISSBLKEY'
    TableName = 'GrievanceTable'
    TableType = ttDBase
    Left = 249
    Top = 180
  end
end
