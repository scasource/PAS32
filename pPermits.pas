...
type
  ...
  public
    ...
    _TableName, SwisSBLKey, TaxRollYr : String;
...
uses GlblVars, PASTypes, WinUtils, PASUTILS, Utilitys,
     GlblCnst, DataAccessUnit, PPermits_Subform,
     Preview;  {Report preview form}

{$R *.DFM}

{=====================================================================}
Procedure TPermitsForm.CreateParams(var Params: TCreateParams);
...
{====================================================================}
Procedure TPermitsForm.SetFocusTimerTimer(Sender: TObject);
...
{====================================================================}
Procedure TPermitsForm.MainListViewCompare(    Sender : TObject;
                                               Item1,
                                               Item2 : TListItem;
                                               Data : Integer;
                                           var Compare : Integer);
...
{====================================================================}
Procedure TPermitsForm.MainListViewColumnClick(Sender : TObject;
                                               Column : TListColumn);
begin
...
begin
  MainQuery.SQL.Clear;
  MainQuery.SQL.Add('Select * from ' + PASPermitsTableName);
  MainQuery.SQL.Add('where SwisSBLKey = ' + '''' + SwisSBLKey + '''');
  MainQuery.SQL.Add('Order by PermitDate');
  MainQuery.Open;

end;  {LoadDataForParcel}

{====================================================================}
Procedure TPermitsForm.FillInMainListView;

begin
  LoadDataForParcel(MainQuery, SwisSBLKey);

  FillInListView(MainListView, MainQuery,
                 ['PermitNumber', 'PermitDate', 'Cost', 'WorkDescription',
                  'COIssued', 'Inspected', 'Entered', 'Permit_ID'],
                 True, False);
...
{====================================================================}
Procedure TPermitsForm.InitializeForm;
...
{==============================================================}
Procedure TPermitsForm.ShowSubform(_SwisSBLKey : String;
                                   _Permit_ID : Integer;
                                   _EditMode : String);

begin
  try
    PermitsEntryDialogForm := TPermitsEntryDialogForm.Create(nil);

    PermitsEntryDialogForm.InitializeForm(_SwisSBLKey, _Permit_ID, _EditMode);

    If (PermitsEntryDialogForm.ShowModal <> idCancel)
      then FillInMainListView;

  finally
    PermitsEntryDialogForm.Free;
  end;

end;  {ShowSubform}

{==============================================================}
Procedure TPermitsForm.MainListViewClick(Sender: TObject);
...
begin
  try
    Permit_ID := StrToInt(GetColumnValueForItem(MainListView, 7, -1));
  except
    Permit_ID := 0;
  end;

  If _Compare(Permit_ID, 0, coGreaterThan)
    then
      begin
        ...
        ShowSubform(SwisSBLKey, Permit_ID, _EditMode);
        FillInMainListView;
        SelectListViewItem(MainListView, Permit_ID);
...
{==============================================================}
Procedure TPermitsForm.MainListViewKeyPress(    Sender: TObject;
                                              var Key: Char);
...
{==============================================================}
Procedure TPermitsForm.NewButtonClick(Sender: TObject);

begin
  ShowSubform(SwisSBLKey, 0, emInsert);
  FillInMainListView;

end;  {NewButtonClick}

{==============================================================}
Procedure TPermitsForm.DeleteButtonClick(Sender: TObject);

var
  Permit_ID : Integer;

begin
    {FXX01262007-1(2.11.1.14): Incorrect column number for permit UID.}
  ...
  with MainQuery do
    try
      SQL.Clear;
      SQL.Add('Select PermitNumber from ' + _TableName);
  ...
  with MainQuery do
    If (MessageDlg('Do you want to delete permit #' + FieldByName('PermitNumber').AsString + '?',
      ...
      then
        begin
          with MainQuery do
            try
              SQL.Clear;
              SQL.Add('Delete from ' + _TableName);
              SQL.Add('where (Permit_ID = ' + IntToStr(Permit_ID) + ')');
            except
            end;
...
{==============================================================}
Procedure TPermitsForm.CloseButtonClick(Sender: TObject);
...
{====================================================================}
Procedure TPermitsForm.FormClose(    Sender: TObject;
